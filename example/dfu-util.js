...
(function() {
  ...
  document.addEventListener('DOMContentLoaded', event => {
    ...
    function onDisconnect(reason) {
      ...
      connectButton.textContent = "First: Connect to Device";
      ...
      if (device) {
        ...
        navigator.usb.requestDevice({ 'filters': filters }).then(
          async selectedDevice => {
            let interfaces = stm32dfu.DFU.findDeviceDfuInterfaces(selectedDevice);
            if (interfaces.length == 0) {
              console.log(selectedDevice);
              statusDisplay.textContent = "The selected device does not have any USB DFU interfaces.";
            } else if (interfaces.length == 1) {
              await fixInterfaceNames(selectedDevice, interfaces);
              device = await connect(new stm32dfu.Device(selectedDevice, interfaces[0]));
            } else {
              await fixInterfaceNames(selectedDevice, interfaces);
              ...
              async function connectToSelectedInterface() {
                ...
                device = await connect(new stm32dfu.Device(selectedDevice, interfaces[index]));
              }
    ...
    uploadButton.addEventListener('click', async function(event) {
      ...
      if (!device || !device.device_.opened) {
        ...
        try {
          ...
          if (status.state == stm32dfu.DFU.dfuERROR) {
